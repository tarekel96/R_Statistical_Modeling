...
```{r}
ggplot(df, aes(x=log(days), y=number_of_reviews)) +
  geom_point() +
  xlab("log(days)") +
  ylab("number of reviews")
```

The expected number of reviews is proportional to the exposure time, log(days), and the rate of increase in the number of reviews appears to be the same for all units. Therefore an offset on log(days) may be needed in modeling. This can be further tested with the Likelihood Ratio Test (LRT). 
...
```{r}
par(mfrow = c(1, 2))
plot(no_offset_model, 1, main = "Residuals vs Fitted (No Offset)")
plot(full_poisson_model, 1, main = "Residuals vs Fitted (Has Offset)")

```

The left-skewed pattern in the residuals versus fitted values plot for the model that does not contain the offset variable suggests that the model is not fully capturing the variation in the data. Specifically, it could indicate that the model is underestimating the number of low-number-of-reviews properties.The left-skew pattern is still observed in the offset model, but not as apparent. The offset model will be used as it helps with the issue of overdispersion that occurs due the different number of samples for the different feature groups.

#### Q 4.11.3.1 Airbnb in NYC - Coefficient Interpretation
```{r}
# Obtaining coefficient estimates
coef_estimates <- coef(full_poisson_model)
...
- review_scores_cleanliness4: the expected the response variable ($\lambda$ = estimated mean number of reviews) would decrease by a factor of approximately 0.7714 holding all other variables constant.
- review_scores_location8: the expected the response variable ($\lambda$ = estimated mean number of reviews) would increase by a factor of approximately 1.1514 holding all other variables constant.
- review_scores_value9: the expected the response variable ($\lambda$ = estimated mean number of reviews) would decrease by a factor of approximately 4.4573 holding all other variables constant.
